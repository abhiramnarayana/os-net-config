...
_V6_IFCFG_EXPANDED_MULTI = (
    _V6_IFCFG_EXPANDED +
    "IPV6ADDR_SECONDARIES=\"2620:00cf:00cf:bbbb:0000:0000:0000:0064/64 "
    "2620:00cf:00cf:cccc:0000:0000:0000:0064/64\"\n")

_V6_IFCFG_COMPRESSED_MULTI = (
    _V6_IFCFG_COMPRESSED +
...
class TestIfcfgNetConfig(base.TestCase):
    ...
    def test_add_interface_with_v6_expanded(self):
        """Expanded IPv6 input should produce compressed ifcfg output."""
        v6_addr = objects.Address(
            '2620:00cf:00cf:aaaa:0000:0000:0000:0064/64')
        interface = objects.Interface('em1', addresses=[v6_addr])
        self.provider.add_interface(interface)
        self.assertEqual(_V6_IFCFG_COMPRESSED, self.get_interface_config())
...
class TestIfcfgNetConfigApply(base.TestCase):
    ...
    def test_ipv6_format_change_no_restart(self):
        """IPv6 format-only change must not trigger interface restart."""
        tmpdir = tempfile.mkdtemp()
        interface_filename = tmpdir + '/ifcfg-em1'
    ...
    def test_ipv6_format_change_no_restart_multi(self):
        """IPv6 format-only change in secondaries must not trigger restart."""
        tmpdir = tempfile.mkdtemp()
        interface_filename = tmpdir + '/ifcfg-em1'

        file = open(interface_filename, 'w')
        file.write(_V6_IFCFG_EXPANDED_MULTI)
        file.close()
    ...
    def test_ipv6_real_change_detected(self):
        """Actual IPv6 address change must still be detected."""
        old = self.provider.parse_ifcfg(_V6_IFCFG_COMPRESSED)
        different_addr = _BASE_IFCFG + """IPV6INIT=yes
IPV6_AUTOCONF=no
IPV6_SET_SYSCTLS=yes
IPV6_FORCE_ACCEPT_RA=no
IPV6ADDR=2620:cf:cf:bbbb::99/64
"""
        new = self.provider.parse_ifcfg(different_addr)
        changes = self.provider.enumerate_ifcfg_changes(old, new)
        self.assertIn('IPV6ADDR', changes)
        self.assertEqual('modified', changes['IPV6ADDR'])

    def test_enumerate_ipv6_dns_format_no_change(self):
        """enumerate_ifcfg_changes ignores IPv6 DNS format differences."""
        old = {'DNS1': '2620:00cf:00cf:aaaa:0000:0000:0000:0001'}
        new = {'DNS1': '2620:cf:cf:aaaa::1'}
        changes = self.provider.enumerate_ifcfg_changes(old, new)
        self.assertNotIn('DNS1', changes)
